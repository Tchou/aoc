...
let create_digrpah start exit width height period grid =
  ...
  let rec loop grid n ((r, c) as point) =
    if not (graph %? (point, n)) then begin
      ...
      let todo = ref [] in
      let () =
        graph.%{point, n} <- graph.%?{point, n} or []
      in
      List.iter
        (fun (i, j) ->
          let dest = r + i, c + j in
  ...
  let solve_part2 () =
    ...
    let () =
      for i = 0 to period - 1 do
        if graph %? (start, i) then targets1 := (start, i) :: !targets1
      done
    in
    let finish_map1 = Algo.dijkstra graph (start1, round1) !targets1 in
    ...
    let () =
      for i = 0 to period - 1 do
        if graph %? (exit, i) then targets2 := (exit, i) :: !targets2
      done
    in
    let finish_map2 = Algo.dijkstra graph (start2, round2) !targets2 in
    let _, len3 =
      Hashtbl.fold
        (fun p (n, _) (pacc, nacc) -> if n < nacc then p, n else pacc, nacc)
        finish_map2
