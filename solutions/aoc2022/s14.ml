...
module Grid = struct
  ...
  let clean_around g p =
    ...
    List.iter
      (fun d ->
         let q = d ++ p in
         if (not (inside g q)) && can_remove q then g.overflow %- q)
      moves
  ...
  let pp fmt g_ =
    ...
    Array.iter
      (fun b ->
         ...
         |> String.explode
         |> List.iter (function 'o' -> Ansi.(fprintf fmt "%ao%a" bfg yellow clear color)
             | c -> Ansi.fprintf fmt "%c" c);
         Ansi.fprintf fmt "@\n")
      g;
    Ansi.fprintf fmt "overflow: %d@\n" (Hashtbl.length g_.overflow)
end
...
module Sol = struct
  ...
  let solve_part1 () = solve false false
  let solve_part2 () = solve false true
end
...
module SA = struct
  ...
  let solve_part1 () = solve true false
  let solve_part2 () = solve true true
end
